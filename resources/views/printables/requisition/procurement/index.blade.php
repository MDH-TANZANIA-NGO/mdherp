...
@if(access()->user()->designation)
    ...
    <div class="content">
        ...
        <div class="header">
            ...
            <div class="document-title bold-text">{{ strtoupper($requisition->typeCategory->title) }} | {{ $requisition->number }}</div>
        ...
        <div class="container">
            ...
            <div class="summary-info">
                <table width="100%">
                    <tbody>
                    <tr>
                        <td class="text-title fixed-width">ID NO :</td> <td class="fixed-width text-left">{{ $requisition->user->identity_number }}</td>
                    </tr>
                    <tr>
                        <td class="text-title fixed-width">NAME :</td> <td class="fixed-width text-left">{{ $requisition->user->fullname }}</td> <td class="text-title fixed-width">PROJECT :</td> <td class="fixed-width text-left">{{ $requisition->project->title }}</td>
                    </tr>
                    <tr>
                        <td class="text-title fixed-width">EMAIL :</td> <td class="fixed-width text-left">{{ $requisition->user->email }}</td> <td class="text-title fixed-width">PROGRAM AREA :</td> <td class="fixed-width text-left">{{ $requisition->activity->subProgram->title }}</td>
                    </tr>
                    <tr>
        ...
        <div class="container">
            ...
            {{-- START Workflow--}}
            <div class="workflow" style="margin-top: 50px">
                <div class="bold-text">APPROVAL SECTION: </div>
                <table>
                    <thead>
                    <tr>
                        <th class="text-left text-title">LEVEL</th>
                        <th class="text-left text-title">NAME</th>
                        <th class="text-left text-title">DESIGNATION</th>
                        <th class="text-left text-title">REMARKS</th>
                        <th class="text-left text-title">COMMENT</th>
                        <th class="text-left text-title">DATE</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach(collect($requisition->wfTracks)->sortBy('id') as $key => $track)
                        @if($track->users)
                            <tr>
                                <td>{{ $track->wfDefinition->level }}</td>
                                <td>{{ strtolower($track->users->fullname) }}</td>
                                <td>{{ $track->wfDefinition->designation->name }} {{ $track->wfDefinition->unit->name }}</td>
                                @if($track->status == 2)
                                    <td>{{ __('label.rejected') }}</td>
                                @else
                                    <td>{{ $track->wfDefinition->status_description }}</td>
                                @endif
                                <td>{{ $track->comments }}</td>
                                <td>{{ date($track->forward_date)}}</td>
                            </tr>
                        @endif
                    @endforeach
                    </tbody>
                ...
                <div class="h3">
                    <small>
                        <center>
                            <div class="h3">Written Approval, if applicable: If director's signature is unavailable, obtain and attach written approval to this document, e.g., an e-mail message</div>
                        </center>
                    </small>
                </div>
            </div>
