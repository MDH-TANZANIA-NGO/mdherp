...
<script>
    $(function() {
                ...
                --$(".wf-module-select").on("change", function() {
                ...
                } {
                    {
                        --get_module_group($(this).val()).done(function(data) {
                            ...
                            {
                                --
                                var $unregistered_modules = {
                                    {
                                        json_encode($unregistered_modules)
                                    }
                                };
                                --
                            }
        ...
        var oTable = $('#pending-workflow-table').DataTable({
            ...
            columns: [
                ...
                {
                    data: 'description',
                    name: 'wf_definitions.description'
                },
                {
                    data: 'module',
                    name: 'wf_modules.name'
                },
                {
                    data: 'status',
                    name: 'assigned',
            ...
            'rowCallback': function(nRow, aData, iDisplayIndex, iDisplayIndexFull) {
                $(nRow).click(function() {
                    switch (aData['module_group_id']) {
                        ...
                        case 9:
                            /*Program Activity Report*/
                            window.open(base_url + "/fleet/" + aData['resource_uuid'] + '/show', "_blank");
                            break;
                        default:
                            break;
                    }
    ...
    function get_module_group($module) {
        return $.post("{{ url('/') }}/getModuleGroup", {
            'module': $module
        }, function($data) {}, "json");
    }
