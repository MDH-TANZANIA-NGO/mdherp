...
//    }
    ...
    public function getApplicantJobOffer()
    {
        return $this->query()->select([
            DB::raw("CONCAT_WS(' ',hr_hire_applicants.first_name,hr_hire_applicants.middle_name,hr_hire_applicants.last_name,  hr_hire_applicants.phone) AS full_name"),
            DB::raw('hr_interview_applicants.id AS id'),
        ])
            ->join('hr_hire_applicants', 'hr_hire_applicants.id', 'hr_interview_applicants.applicant_id')
            ->where('hr_interview_applicants.status',1);
