...
{
    use WorkflowProcessLevelActionTrait, WorkflowResourceModificationStageTrait;
    /**
...
//                return $query->resource->status;
    ...
    {
        return $this->query()->whereHas('application')->where('status', 0)->where('user_id', access()->id())->whereHas('wfDefinition', function ($query) {
            $query->whereHas('users', function ($subQuery) {
                $subQuery->where('users.id', access()->id());
            });
        });
    ...
    {
        return $this->query()->where('status', 0)->where('user_id',null)->whereHas('wfDefinition', function ($query) {
            $query->whereHas('users', function ($subQuery) {
                $subQuery->where('users.id', access()->id());
            });
        });
